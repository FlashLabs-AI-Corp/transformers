...
class ChromaGenerationMixin(GenerationMixin):
    ...
    def generate(
        ...
        **kwargs: dict
        ...
        ```python
        >>> from transformers import ChromaProcessor, ChromaForConditionalGeneration

        >>> model_id = "FlashLabs/chroma"
        ...
        >>> model = ChromaForConditionalGeneration.from_pretrained(model_id, device_map=torch_device)
        >>> output = model.generate(**inputs, do_sample=True, use_cache=True, temperature=0.9, top_k=50, max_new_tokens=1000, output_audio=True)
